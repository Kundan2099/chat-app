        ...
        <div class="mt-20 mb-16">
            @foreach ($messages as $message)
                ...
                @if (isset($message['sender']) && $message['sender'] != auth()->used()->name)
                    <div class="clearfix">
                        <div class="bg-gray-300 w-3/4 mx-4 my-2 p-2 rounded-lg">{{ $message->sender->name }}</div>
                    </div>
                @else
                    <div class="clearfix">
                        <div class="bg-green-300 float-right w-3/4 mx-4 my-2 p-2 rounded-lg clearfix">
                          {{-- {{ $message->receiver->name }}</div> --}}
                    </div>
                @endif
            @endforeach
    ...
    <form wire:submit="sendMessage()">
        <div class="fixed w-full flex justify-between bg-green-100" style="bottom: 0px;">
            <textarea class="flex-grow m-2 py-2 px-4 mr-1 rounded-full border border-gray-300 bg-gray-200 resize-none"
                rows="1" {{-- wire:model="message" --}} wire:model="message" placeholder="Message..." style="outline: none;"></textarea>
